...
namespace Engine.UI
{
    ...
    public class Container : Group
	{
		...
		public static Container Create(Element element)
        {
		...
		public override void Draw( float parentAlpha)
		{
			...
			if (transform)
			{
				...
				DrawBackground( parentAlpha, 0, 0);
				if (_clip)
				{
					//batcher.flush();
			...
			{
				DrawBackground( parentAlpha, GetX(), GetY());
		...
		protected void DrawBackground( float parentAlpha, float x, float y)
		{
			if (_background == null)
				return;

			_background.Draw( x, y, GetWidth(), GetHeight(),  new Color(color.r,color.g,color.b, (int)(color.a * parentAlpha)));
		}
		...
		public override void DebugRender()
		{
			...
			{
				Raylib_cs.Raylib.DrawRectangleLinesEx(new(x, y, width, height), 1, Color.RED);
				Raylib_cs.Raylib.DrawTextPro(Raylib.GetFontDefault(), GetType().Name, new Vector2(x, y), Vector2.Zero, rotation, 15, 1, Color.RED);
			}
		}
	}
}

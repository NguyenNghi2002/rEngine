...
{
	/// <summary>
	/// A group that sizes and positions children using table constraints. By default, <see langword="virtual " cref="Element.GetTouchable()"/>  is
	/// <see cref="Touchable.ChildrenOnly"/>.
	///  <br/>
	...
	public class Table : Group
	{
		...
		public override void Draw( float parentAlpha)
		{
			...
			{
				DrawBackground( parentAlpha, x, y);
		...
		/// drawable.
		/// </summary>
		protected virtual void DrawBackground( float parentAlpha, float x, float y)
		{
			if (_background == null)
				return;

			var col = new Color(color.r, color.g, color.b, (int)(color.a * parentAlpha));
			_background.Draw( x, y, width, height, col);
		}

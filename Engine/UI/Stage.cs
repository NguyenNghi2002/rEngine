...
using Raylib_cs;
using Buttons = Raylib_cs.GamepadButton;
using Keys = Raylib_cs.KeyboardKey;
using Camera = Raylib_cs.Camera2D;
...
namespace Engine.UI
{
	...
	{
		/// <summary>
		/// This will be null for outside of component usuage
		/// </summary>
		...
		/// </summary>
		public Buttons GamepadActionButton = Buttons.GAMEPAD_BUTTON_LEFT_FACE_UP;
		...
		/// </summary>
		public Keys KeyboardActionKey = Keys.KEY_ENTER;

		Group root;
		public Camera CameraUI = default;
		...
		float _keyRepeatStartTime = 0.4f;
		float _keyRepeatTime = 0.07f;
		Keys _repeatKey;
		...
		{
			return _mousePosition; //Raylib.GetMousePosition();//Entity != null && !IsFullScreen ? Input.ScaledMousePosition : Input.RawMousePosition.ToVector2();
		}
        ...
        public void Update()
        {
            ...
            if (_isGamepadFocusEnabled)
                UpdateGamepadState();
            UpdateKeyboardState();
            UpdateInputMouse(Input.MousePosition);
        ...
        /// </summary>
        void UpdateInputMouse(Vector2 mousePosition)
        {
			// consolidate input checks so that we can add touch input easily later
			this._mousePosition = mousePosition;
			var currentMousePosition = mousePosition;//GetMousePosition();
        ...
        void HandleMouseWheel(Element mouseOverElement)
        {
            ...
            if (listener != null && listener.OnMouseScrolled((int)Input.MouseWheelDelta))
                return;
		...
		void UpdateKeyboardState()
		{
			...
			var currentHeldKeys = Input.CurrentKeysDown;
			// keys down
			for (var i = 0; i < currentHeldKeys.Length; i++)
			{
				var key = currentHeldKeys[i];
				if (!_lastPressedKeys.Contains(key))
				{
					...
					if (!InputUtils.IsAltDown())
					{
						...
						if (c.HasValue)
						{
							...
							{
								_repeatKey = key;
								_keyRepeatTimer = Core.Schedule(_keyRepeatStartTime, false, this, t =>
								{
									_keyRepeatTimer = Core.Schedule(_keyRepeatTime, true, this, t =>
									{
										var self = t.Context as Stage;
										if (self._keyboardFocusElement != null)
											self._keyboardFocusElement.KeyPressed(_repeatKey, (char)_repeatKey);
									});
								});
			...
			for (var i = 0; i < _lastPressedKeys.Length; i++)
			{
				...
				if (!currentHeldKeys.Contains(key))
				{
			...
			_lastPressedKeys = currentHeldKeys;
#endif
		}
		...
		{
            if (Entity != null && !IsFullScreen)
                return  Entity.Scene.Resolution.X;
		...
		{
            if (Entity != null && !IsFullScreen)
                return Entity.Scene.Resolution.Y;
		...
		{
			//if (Camera.ReferenceEquals(null))
				return screenCoords;
			//return CameraUI.ScreenToWorldPoint(screenCoords);
		}
		...
		{
			return CameraUI.WorldToScreenPoint(stageCoords);
		}

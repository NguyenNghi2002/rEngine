...
namespace Engine.UI
{
	public class ProgressBar : Element
	{
		...
		public override void Draw(float parentAlpha)
		{
			...
			var color = new Color(this.color.r, this.color.b, this.color.g, (int)(this.color.a * parentAlpha));
			...
			if (_vertical)
			{
				...
				if (bg != null)
				{
					bg.Draw( x + (int) ((width - bg.MinWidth) * 0.5f), y, bg.MinWidth, height, color);
				...
				if (knobBefore != null)
				{
					...
					if (bg != null)
						offset = bgTopHeight;
					knobBefore.Draw( x + ((width - knobBefore.MinWidth) * 0.5f), y + offset,
						knobBefore.MinWidth,
				...
				if (knobAfter != null)
				{
					knobAfter.Draw( x + ((width - knobAfter.MinWidth) * 0.5f), y + position + knobHeightHalf,
						knobAfter.MinWidth, height - position - knobHeightHalf, color);
				}

				if (knob != null)
					knob.Draw(x + (int) ((width - knobWidth) * 0.5f), (int) (y + position), knobWidth,
						knobHeight, color);
			}
			else
			{
				...
				if (bg != null)
				{
					bg.Draw( x, y + (int) ((height - bg.MinHeight) * 0.5f), width , bg.MinHeight, color);
				...
				if (knobBefore != null)
				{
					...
					if (bg != null)
						offset = bgLeftWidth;
					knobBefore.Draw( x + offset, y + (int) ((height - knobBefore.MinHeight) * 0.5f),
						(int) (position + knobWidthHalf), knobBefore.MinHeight, color);
				}

				if (knobAfter != null)
				{
					knobAfter.Draw( x + (int) (position + knobWidthHalf),
						y + (int) ((height - knobAfter.MinHeight) * 0.5f),
						width - (int) (position + knobWidthHalf), knobAfter.MinHeight, color);
				}

				if (knob != null)
					knob.Draw((int) (x + position), (int) (y + (height - knobHeight) * 0.5f), knobWidth,
						knobHeight, color);
			}
		}

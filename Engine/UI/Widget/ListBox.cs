...
namespace Engine.UI
{
	...
	public class ListBox<T> : Element, IInputListener where T : class
	{
		...
		public override void Layout()
		{
			...
			_textOffsetY = selectedDrawable.TopHeight ;
		...
		public override void Draw( float parentAlpha)
		{
			...
			if (background != null)
			{
				background.Draw( x, y, width, height, color);
			...
			for (var i = 0; i < _items.Count; i++)
			{
					 ...
					 itemY >= _cullingArea.Value.y))
				{
					...
					if (selected)
					{
						selectedDrawable.Draw( x, y + itemY, width, _itemHeight, color);
						fontColor = selectedFontColor;
					}
					else if (i == _hoveredItemIndex && _style.HoverSelection != null)
					{
						_style.HoverSelection.Draw( x, y + itemY, width, _itemHeight, color);

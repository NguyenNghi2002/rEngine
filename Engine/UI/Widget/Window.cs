...
namespace Engine.UI
{
	...
	public class Window : Table, IInputListener
	{
		...
		public event Action<Window> OnBeginResize;
		public event Action<Window> OnEndResize;
		...
		public Window(string title, WindowStyle style)
		{
			...
			titleLabel = new Label(title, new LabelStyle(style.TitleFont.Value, style.TitleFontColor, style.TitleFontScale));
		...
		public Window SetStyle(WindowStyle style)
		{
			...
			labelStyle.FontScale = style.TitleFontScale;
		...
		public override void Draw(float parentAlpha)
		{
			...
			if (style.StageBackground != null)
			{
				...
				DrawStageBackground(parentAlpha, GetX() + stagePos.X, GetY() + stagePos.Y,
		...
		protected void DrawStageBackground(float parentAlpha, float x, float y, float width, float height)
		{
			style.StageBackground.Draw( x, y, width, height, ColorExt.Create(color, (int)(color.a * parentAlpha)));
		}


		protected override void DrawBackground( float parentAlpha, float x, float y)
		{
			base.DrawBackground( parentAlpha, x, y);
	...
	public class WindowStyle
	{
		...
		/** Optional. */
		public float TitleFontScale = 20f;
		...
		public WindowStyle()
		{
			TitleFont = Raylib.GetFontDefault();
		}
        ...
        public WindowStyle(BitmapFont? titleFont, Color titleFontColor, IDrawable background, float titleFontScale)
        {
            TitleFont = titleFont ?? Raylib.GetFontDefault();
            Background = background;
            TitleFontColor = titleFontColor;
            TitleFontScale = titleFontScale;
        }

        public static WindowStyle CreateRaylib()
        {
			return new WindowStyle()
			{
				StageBackground = new PrimitiveDrawable(Raylib.Fade(Color.BLANK, 0.3f), null),
				Background = new PrimitiveDrawable(UIDefault.BaseUp,UIDefault.OutlineUp),
